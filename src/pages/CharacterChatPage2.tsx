import {
  ...
  const CharacterChatPage2: React.FC = () => {
    ...
    const [isToastVisibleMessage, setIsToastVisibleMessage] = useState<boolean>(false);
    ...
    useEffect(() => {
      ...
      socket.on(authToken + id + 'new_message', (msg: { text: string; is_user: number }) => {
        console.error('Received new message:', msg);
        setIsLoadingBotMessage(false);

        setHtmlContent(msg.text)
  ...
  const displayToastSendMessage = (message: string, type: string) => {
    console.error("In display toast")
    setToastMessage2(message);
    setIsToastVisibleMessage(true);
    setTimeout(() => {
        setIsToastVisibleMessage(false);
    }, 4000);
  };
    ...
    try {
        ...
        const fcm = localStorage.getItem('fcmToken');
            ...
            const lastBotMessage = formattedMessages
                ...
                .slice(-1)[0]; // Gets the last bot message




                console.log("lastbot messages::::::", lastBotMessage.content)


    
            setHtmlContent(lastBotMessage.content)
  ...
  const handleBackButtonClick = () => {
    ...
    const handleOutsideClick = () => {
        if (inputRef.current) {
            inputRef.current.blur(); // Remove focus when clicking outside
        }
    };
  ...
  const [bubbles, setBubbles] = useState<JSX.Element[]>([]);
    ...
    useEffect(() => {
        ...
        const handleClickOutside = (event: MouseEvent) => {
          if (
            isKeyboardOpen2 && // Only hide if the keyboard is open
            inputRef.current &&
    ...
    return (
      <IonPage className='ionPage'>
        ...
        <div style={{
              transform: `translateY(-${keyboardHeight}px)`,
              transition: 'transform 255ms ease-out',
            }} ref={messageContainerRef} className="html-render" dangerouslySetInnerHTML={{ __html: htmlContent }} onClick={handleOutsideClick}>
  ...
  >
    <div className="input-container">
      ...
      onClick={async (e) => {
        e.stopPropagation();
        await Keyboard.hide(); // Hide the keyboard
        sendMessage();
      }}
        fill="clear"
  ...
  export default CharacterChatPage2; 